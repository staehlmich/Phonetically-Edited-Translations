...
def grapheme_to_phoneme(pronunciation_dic:dict, src_file: str, trg_file: str, output_file:str):
    """
    ...
    @param src_file: file with source phrases.
    @param trg_file: file with target phrases.
    @param output_file: file containing lines with:
    phonetized phrase, source phrase, target phrase
    <<W><IY>>: phonetized token.
    @return:
    """
    with open(src_file, "r", encoding="utf-8") as src, \
        ...
        for n, line in enumerate(src):
            ...
            for token in line:
                ...
                new_line = new_line + "<{}>".format(pronunciation_dic[token.lower()])+" "
                ...
                if pronunciation_dic[token.lower()] == "<UNK>":
                    ...
                    if token.isdigit():
                        new_line = new_line + \
                               "<{}>".format(
                                   token) + " "
                    #Unknown phonetic representation
                    else:
                        new_line = new_line + \
                                  "<UNK>"

            #Filter out phrases with unphoneitzed tokens.
            if "<UNK>" not in new_line:
                outsrc = src[n].rstrip()
                outtrg = trg[n].rstrip()
                # Write line and newline char to outfile.
                # [:-1] to avoid writing last whitespace
                outfile.write(new_line[:-1]+","+outsrc+","+outtrg+"\n")
