...
def add_delta_deltas(filterbanks, name=None):
    ...
    filterbanks = tf.nn.conv2d(
        input=filterbanks, filters=delta_filter_stack, strides=[1, 1, 1, 1], padding="SAME", data_format="NHWC",
        name=name)
    return filterbanks
...
def compute_mel_filterbank_features(
    ...
    wav_lens = tf.reduce_max(
        input_tensor=tf.expand_dims(tf.range(tf.shape(input=waveforms)[1]), 0) *
        tf.cast(tf.not_equal(waveforms, 0.0), dtype=tf.int32),
        axis=-1) + 1
    if dither > 0:
        waveforms += tf.random.normal(tf.shape(input=waveforms), stddev=dither)
    if preemphasis > 0:
    ...
    masks = tf.cast(tf.less_equal(
        tf.expand_dims(tf.range(tf.shape(input=stfts)[1]), 0),
...
def preprocess_example(example, hparams):
    ...
    if p.audio_preproc_in_bottom:
        ...
        var_epsilon = 1e-09
        mean = tf.reduce_mean(input_tensor=mel_fbanks, keepdims=True, axis=1)
        variance = tf.reduce_mean(input_tensor=tf.math.squared_difference(mel_fbanks, mean),
...
def build_model(hparams, bs=64):
    ...
    for pidx in range(bs):
        waveform = tf.compat.v1.placeholder(tf.float32, [None], "waveform_{}".format(pidx))
...
if __name__ == "__main__":
    ...
    hparams = get_hparams(lambda : None)
